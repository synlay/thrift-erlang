...
parse_handler_options_test_() ->
    ...
    CorrectHandlerTestFunction = fun() ->
        ?assertMatch({thrift_socket_server,_,_,_,_,_,_,_,_,_,_,_,_}, thrift_socket_server:parse_options([{handler, CorrectServiceHandlerOptionList}])),
        {thrift_socket_server,_,_,_, HandlerList,_,_,_,_,_,_,_,_} = thrift_socket_server:parse_options([{handler, CorrectServiceHandlerOptionList}]),
        lists:foreach(fun
    ...
    [
     ...
     {"Parse the handler option as a non multiplexed service handler", ?_assertMatch({thrift_socket_server,_,_,_,?MODULE,_,_,_,_,_,_,_,_}, thrift_socket_server:parse_options([{handler, ?MODULE}]))},
...
parse_service_options_test_() ->
    ...
    CorrectServiceModuleTestFunction = fun() ->
        ?assertMatch({thrift_socket_server,_,_,_,_,_,_,_,_,_,_,_,_}, thrift_socket_server:parse_options([{service, CorrectServiceModuleOptionList}])),
        {thrift_socket_server,_, ServiceModuleList,_,_,_,_,_,_,_,_,_,_} = thrift_socket_server:parse_options([{service, CorrectServiceModuleOptionList}]),
        lists:foreach(fun
    ...
    [
     ...
     {"Try to parse the service option twice", ?_assertThrow(_, thrift_socket_server:parse_options([{service, "ServiceTest", ?MODULE}, {service, CorrectServiceModuleOptionList}]))},
     {"Parse a service module for a non multiplexed service", ?_assertMatch({thrift_socket_server,_,?MODULE,_,_,_,_,_,_,_,_,_,_}, thrift_socket_server:parse_options([{service, "ServiceTest", ?MODULE}]))},
